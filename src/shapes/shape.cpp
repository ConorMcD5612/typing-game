...
Shape::Shape(Shader &shader, glm::vec2 pos, glm::vec2 size, struct color color) :
    shader(shader), pos(pos), size(size), _color(color) {}

Shape::Shape(Shape const& other) :
    shader(other.shader), pos(other.pos), size(other.size), _color(other._color) {}
...
void Shape::setUniforms() const {
    ...
    this->shader.setVector4f("shapeColor", _color.vec);
}
...
void Shape::setColor(struct color c)    { _color = c; }
void Shape::setColor(vec4 c)     { _color.vec = c; }
void Shape::setColor(vec3 c)     { _color.vec = vec4(c, 1.0); }
void Shape::setRed(float r)      { _color.red = r; }
void Shape::setGreen(float g)    { _color.green = g; }
void Shape::setBlue(float b)     { _color.blue = b; }
void Shape::setOpacity(float a)  { _color.alpha = a; }
...
vec3 Shape::getColor3() const   { return {_color.red, _color.green, _color.blue}; }
vec4 Shape::getColor4() const   { return _color.vec; }
float Shape::getRed() const     { return _color.red; }
float Shape::getGreen() const   { return _color.green; }
float Shape::getBlue() const    { return _color.blue; }
float Shape::getOpacity() const { return _color.alpha; }
